...
er_a = {
  ...
  buildEmptyTableEntry (accId) {
    ...
    const emptyMessageRow = document.createElement('div')
  ...
  buildEmptyAccTransactionsSvg () {
    ...
    const pathElement21 = document.createElementNS('http://www.w3.org/2000/svg', 'path')
    ...
    const pathElement31 = document.createElementNS('http://www.w3.org/2000/svg', 'path')
    ...
    const pathElement41 = document.createElementNS('http://www.w3.org/2000/svg', 'path')
  ...
  buildAccountHeader (account) {
    ...
    const accountTit = document.createElement('div')
    ...
    // Title
    const accountTitle = document.createElement('a')
  ...
  populateAccountInfo (bankAccountsInfo) {
    const bAFragment = document.createDocumentFragment()

    // Bank Account Heading
    const bAHRow = document.createElement('div')
    bAHRow.classList = 'row'

    const bAHeading = document.createElement('h4')
    ...
    const bAManage = document.createElement('a')
    ...
    const bABody = document.createElement('div')
    ...
    // Bank Account Footer
    const bAFooter = document.createElement('button')
    ...
    const accountPickerModal = document.getElementById('accountPickerWrapper')
  ...
  populateBankAccountInfo (bankAccount, count) {
    ...
    // Bank Account Name
    const bAName = document.createElement('div')
    ...
    const bABalance = document.createElement('div')
  ...
  populateEmptyAccountInfo () {
    const emptyAccountFragment = document.createDocumentFragment()
    ...
    const svgWrapperTwo = document.createElement('div')
    svgWrapperTwo.classList = 'vertical-center-svg'

    const syncSVGTwo = document.createElementNS('http://www.w3.org/2000/svg', 'svg')
    ...
    const tenColTwo = document.createElement('div')
    tenColTwo.classList = 'font-weight-bold'

    const unsyncTitle = document.createElement('div')
    ...
    const svgElemWrapThree = document.createElement('div')
    svgElemWrapThree.classList = 'col-lg-2 vertical-center-svg pr-2 pl-2 account-info-color'

    const svgElemThree = document.createElementNS('http://www.w3.org/2000/svg', 'svg')
    svgElemThree.setAttribute('viewBox', '0 0 16 16')

    const pathElemThree = document.createElementNS('http://www.w3.org/2000/svg', 'path')
    ...
    const colTenThree = document.createElement('div')
    ...
    const restOfTheText = document.createElement('span')
  ...
  unSyncedAccount () {
    const unsyncedDocumentFragment = document.createDocumentFragment()

    const unsyncFormWrapper = document.createElement('div')
    ...
    // Choose Type
    const chooseTypeWrapper = document.createElement('div')
    chooseTypeWrapper.classList = 'chooseTypeWrapper text-left'

    const chooseTypeLabel = document.createElement('label')
    ...
    const displaySelected = document.createElement('button')
    ...
    // Drop Down Menu
    const budgetHeading = document.createElement('label')
    ...
    const sadata = isNotEmpty(window.translationData) ? window.translationData.account.dynamic.add.savingsacc : 'Savings Account'
    const cadata = isNotEmpty(window.translationData) ? window.translationData.account.dynamic.add.currentacc : 'Current Account'
    const cashdata = isNotEmpty(window.translationData) ? window.translationData.account.dynamic.add.cash : 'Cash'
    const assetdata = isNotEmpty(window.translationData) ? window.translationData.account.dynamic.add.assets : 'Assets'
    const creditcarddata = isNotEmpty(window.translationData) ? window.translationData.account.dynamic.add.creditcard : 'Credit Card'
    const liabilitydata = isNotEmpty(window.translationData) ? window.translationData.account.dynamic.add.liability : 'Liability'
    // Savings
    const savingsAnchor = document.createElement('a')
    ...
    // Cash
    const cashAnchor = document.createElement('a')
    ...
    // Assets
    const assetsAnchor = document.createElement('a')
    ...
    // Drop Down Menu 2
    const debtHeading = document.createElement('label')
    ...
    // Credit card
    const creditCardAnchor = document.createElement('a')
    ...
    // Liability
    const liabilityAnchor = document.createElement('a')
    ...
    const accountTypeError = document.createElement('div')
    ...
    // Name Of account
    const accountNameWrapper = document.createElement('div')
    ...
    // Account Balance
    const accountBalWrapper = document.createElement('div')
    accountBalWrapper.classList = 'accountBalWrapper text-left'

    const accountBalLabel = document.createElement('label')
    ...
    const accountBalInput = document.createElement('input')
    ...
    const accountBalErr = document.createElement('div')
...
(function scopeWrapper ($) {
  ...
  const showAccountsDiv = document.getElementById('showAccounts')
  if (isNotEmpty(showAccountsDiv)) {
    showAccountsDiv.addEventListener('click', function () {
      const accountPickerClass = document.getElementById('accountPickerWrapper').classList
  ...
  function closeShowAccountsModal (event) {
    const accountPicker = document.getElementById('accountPickerWrapper')
    const showAccountsDiv = document.getElementById('showAccounts')
  ...
  $(document).on('click', '.accountType', function () {
    ...
    accountSubTypeToType[creditcarddata] = 'DEBT'
    accountSubTypeToType[liabilitydata] = 'DEBT'
    const selectedAT = this.getAttribute('data-target')
    const accountTypeECL = document.getElementById('accountTypeErr').classList
    const changeClrBtn = document.getElementsByClassName('changeBtnClr')[0].classList
    const accCfrmBtn = document.getElementsByClassName('swal2-confirm')[0]
    const accountBalErr = document.getElementById('accountBalErr').classList
    const accBalance = document.getElementById('accountBal').value

    // Set Text
    const accountChosen = document.getElementsByClassName('accountChosen')[0]
  ...
  $(document).on('click', '#syncAccountWrap', function () {
    const feature = isNotEmpty(window.translationData) ? window.translationData.account.dynamic.feature : 'This feature is coming soon!'
    showNotification(feature, 'info')
  })

  // Click on Add unsynced account
  $(document).on('click', '#unsyncedAccountWrap', function () {
    const create = isNotEmpty(window.translationData) ? window.translationData.account.dynamic.create : 'Create Account'
    const addtitle = isNotEmpty(window.translationData) ? window.translationData.account.dynamic.addtitle : 'Add Unsynced Account'
    // Show Sweet Alert
    Swal.fire({
      title: addtitle,
      ...
      if (result.value) {
        const accountSubType = document.getElementsByClassName('accountChosen')[0].getAttribute('data-target')
        let accountBalance = parseInt(document.getElementById('accountBal').value)
        const accountType = accountSubTypeToType[accountSubType]
        ...
        ajaxData.onSuccess = function (result) {
          ...
          const accountHeaderNew = er_a.buildAccountHeader(result)
  ...
  $('#accountPickerWrapper').on('click', '.bARow', function () {
    ...
    const bnkAccountId = currentElem.getAttribute('data-target')
    ...
    ajaxData.onSuccess = function (result) {
      ...
      for (let i = 0, length = window.allBankAccountInfoCache.length; i < length; i++) {
        const bankAccount = window.allBankAccountInfoCache[i]
      ...
      const bARows = document.getElementsByClassName('bARow')
      // Remove class from list
      for (let i = 0, length = bARows.length; i < length; i++) {
        const rowElem = bARows[i]
  ...
  function closeAccountPopup () {
    const accountPickerWrapper = document.getElementById('accountPickerWrapper').classList
    // Close the popup
  ...
  function populateKnowMore () {
    // Account Picker Wrapper
    const accountPickerModal = document.getElementById('accountPickerWrapper')
    const knowMoreFrag = document.createDocumentFragment()
    ...
    const accountInfoDesc = document.createElement('div')
    ...
    // Table Responsive
    const tableReponsive = document.createElement('div')
    ...
    // First Empty HEad
    const firstTableHead = document.createElement('div')
    firstTableHead.classList = 'w-60 d-table-cell'
    tableHeading.appendChild(firstTableHead)

    // Second Table Head
    const secTableHead = document.createElement('div')
    ...
    // Third Table Head
    const thirdTableHead = document.createElement('div')
    ...
    // Table First Row
    const firstTableRow = document.createElement('div')
    firstTableRow.classList = 'd-table-row'

    const emptyFC1 = document.createElement('div')
    emptyFC1.classList = 'd-table-cell'
    firstTableRow.appendChild(emptyFC1)

    const titleSC1 = document.createElement('div')
    ...
    const titlsTC1 = document.createElement('div')
    ...
    // Table Six Row
    const SixthTableRow = document.createElement('div')
    SixthTableRow.classList = 'd-table-row'

    const emptyFC6 = document.createElement('div')
    ...
    const titleSC6 = document.createElement('div')
    ...
    const titlsTC6 = document.createElement('div')
    ...
    // Table Second Row
    const secondTableRow = document.createElement('div')
    secondTableRow.classList = 'd-table-row'

    const emptyFC2 = document.createElement('div')
    ...
    const titleSC2 = document.createElement('div')
    ...
    const titlsTC2 = document.createElement('div')
    ...
    // Table Third Row
    const ThirdTableRow = document.createElement('div')
    ThirdTableRow.classList = 'd-table-row'

    const emptyFC3 = document.createElement('div')
    ...
    const titleSC3 = document.createElement('div')
    ...
    const titlsTC3 = document.createElement('div')
    ...
    // Table Fourth Row
    const FourthTableRow = document.createElement('div')
    FourthTableRow.classList = 'd-table-row'

    const emptyFC4 = document.createElement('div')
    ...
    const titleSC4 = document.createElement('div')
    ...
    const titlsTC4 = document.createElement('div')
    ...
    // Table Fifth Row
    const FifthTableRow = document.createElement('div')
    FifthTableRow.classList = 'd-table-row'

    const emptyFC5 = document.createElement('div')
    ...
    const titleSC5 = document.createElement('div')
    ...
    const titlsTC5 = document.createElement('div')
  ...
  $('#accountPickerWrapper').on('click', '.manageBA', function () {
    ...
    const manageaccount = isNotEmpty(window.translationData) ? window.translationData.account.dynamic.manage : 'Account'
    window.sortingTransTable = manageaccount
    // Check if the transactions page is active
    const sidebar = document.getElementById('transaction-dashboard-sidebar')
