...
(function scopeWrapper ($) {
  ...
  const settingsPgDD = document.getElementById('settingsPgDD')
  ...
  function updateRelevantFields (element) {
    const chooseCtyId = 'chosenCountryInpautocomplete-list'
    const chooseCrncyId = 'chosenCurrencyInpautocomplete-list'
  ...
  function patchWallets (chosenCurrencyMW, event, valObj) {
    ...
    jQuery.ajax({
      ...
      success: function (result) {
        ...
        const inpBtnSrch = event.parentElement.id.replace('autocomplete-list', '')
        const inpSearchEl = document.getElementById(inpBtnSrch)
        const itemWithWallet = document.getElementById(valObj.parentElId)
        // First Child Input value
        const oldValText = itemWithWallet.firstElementChild.lastElementChild.value
  ...
  function populateSettings () {
    ...
    /**
         ...
         **/
    const currentExportFileFormat = exportFileFormatObj[currentUser.exportFileFormat]
    ...
    window.countries = []
    window.lToC = {}
    const locToCou = window.localeToCountry.localeToCountry
    ...
    $('#chosenCurrencyDropdown').on('shown.bs.dropdown', function (event) {
      const currencyInp = document.getElementById('chosenCurrencyInp')
    ...
    $('#chosenCountryDropdown').on('shown.bs.dropdown', function (event) {
      const countryInp = document.getElementById('chosenCountryInp')
      // Input change focus to the country search bar
      countryInp.focus()
      // Trigger input event
      const eventInp = new Event('input', {
