...
Object.defineProperties(CUSTOM_DASHBOARD_CONSTANTS, {
  bankAccountUrl: {
    value: window._config.api.invokeUrl + '/bank-accounts',
    writable: false,
    configurable: false
  },
  overviewUrl: {
    value: window._config.api.invokeUrl + '/overview',
    writable: false,
    configurable: false
  },
  fetchCategoriesUrl: {
    value: window._config.api.invokeUrl + '/categories',
    writable: false,
    configurable: false
  },
  transactionAPIUrl: {
    value: window._config.api.invokeUrl + '/transactions',
    writable: false,
    configurable: false
  },
  budgetAPIUrl: {
    value: window._config.api.invokeUrl + '/budgets',
    writable: false,
    configurable: false
  },
  ...
  transactionDashboardId: {
    value: 'transaction-dashboard-sidebar',
    writable: false,
    configurable: false
  },
  goalDashboardId: {
    value: 'goal-dashboard-sidebar',
    writable: false,
    configurable: false
  },
  budgetDashboardId: {
    value: 'budget-dashboard-sidebar',
    writable: false,
    configurable: false
  },
  settingsDashboardId: {
  ...
  (function scopeWrapper ($) {
    ...
    function readCookie () {
      ...
      if (document.cookie != '') {
        ...
        const sidebarActiveCookie = er.getCookie('sidebarMini')
        ...
        // Get the value from the name=value pair
        const cookieCurrentPage = er.getCookie('currentPage')
    ...
    function fetchCurrentPage (id) {
      ...
      const navBarDiv = document.getElementsByClassName('navbar')
    ...
    function buildYear (selectedYear) {
      const monthPickerYear = document.getElementsByClassName('monthPickerYear')
    ...
    function chooseCurrentMonth () {
      ...
      const monthPicker = document.getElementById('monthPicker-' + currentMonth)
      if (isNotEmpty(monthPicker)) {
        monthPicker.classList.add('monthPickerMonthCurrent', 'monthPickerMonthSelected')
      }
    }

    // Click event for month picker
    const monthPickerDisplayDiv = document.getElementById('monthPickerDisplay')
    if (isNotEmpty(monthPickerDisplayDiv)) {
      monthPickerDisplayDiv.addEventListener('click', function (e) {
        e.stopPropagation()
        const dateControlClass = document.getElementById('dateControl').classList
        // Change the SVG to down arrow or up arrow
        const overvierDateArrow = document.getElementsByClassName('overviewDateArrow')[0].classList
        if (!dateControlClass.contains('d-none')) {
          ...
          const selectedMonthDiv = document.getElementsByClassName('monthPickerMonthSelected')
    ...
    const monthPickerPrev = document.getElementById('monthPickerPrev')
    ...
    function calcCurrentMonthInPopover () {
      const currentMonth = document.getElementsByClassName('monthPickerMonthCurrent')
      const currentDate = new Date()
      const currentDateAsMonth = Number(currentDate.getMonth()) + 1
    ...
    function calcCurrentMonthSelected () {
      ...
      if (selectedMonthDiv.length === 0 && Number(positionMonthCache.slice(-4)) == popoverYear) {
        const positionElement = Number(positionMonthCache.slice(0, 2))
    ...
    function closeMonthPickerModal (event) {
      const dateControlDiv = document.getElementById('dateControl')
      const overvierDateArrow = document.getElementsByClassName('overviewDateArrow')[0].classList
      const monthPickerPrev = document.getElementById('monthPickerPrev')
      const monthPickerNext = document.getElementById('monthPickerNext')
    ...
    function updateExistingDateInMonthPicker () {
      if (isNotEmpty(window.datesCreated)) {
        for (let countGrouped = 0, length = window.datesCreated.length; countGrouped < length; countGrouped++) {
          const date = window.datesCreated[countGrouped]

          // Convert the date key as date
          const dateAsDate = new Date(date.dateId.substring(5, date.dateId.length))

          if (popoverYear != dateAsDate.getFullYear()) {
            continue
          }

          const currentMonth = dateAsDate.getMonth() + 1
          const monthPickerClass = document.getElementById('monthPicker-' + currentMonth).classList
    ...
    function setCurrentMonthAndYID () {
      const overviewHeading = document.getElementById('overviewMonthHeading')
      const overviewYearHeading = document.getElementsByClassName('overviewYearHeading')[0]

      const currentDate = new Date()
      if (isNotEmpty(overviewHeading)) {
        overviewHeading.textContent = window.months[currentDate.getMonth()]
      }
      if (isNotEmpty(overviewYearHeading)) {
        overviewYearHeading.textContent = currentDate.getFullYear()
      }
    }
    ...
    const dashboardFullScreen = document.getElementById('dashboard-util-fullscreen')
    ...
    /* Minimize sidebar */
    const minimizeSidebarDiv = document.getElementById('minimizeSidebar')
    if (isNotEmpty(minimizeSidebarDiv)) {
      minimizeSidebarDiv.addEventListener('click', function () {
        ...
        /* Create a cookie to store user preference */
        document.cookie = (md.misc.sidebar_mini_active == 1 ? 'sidebarMini=active; expires=' + expirationDate.toGMTString() : 'sidebarMini=inActive; expires=' + expirationDate.toGMTString())
      })
    }

    /* Minimise sidebar */
    function minimizeSidebar () {
      md.misc.sidebar_mini_active == 1 ? ($('body').removeClass('sidebar-mini'), md.misc.sidebar_mini_active = !1) : ($('body').addClass('sidebar-mini'), md.misc.sidebar_mini_active = !0)
    ...
    const mutScrollable = document.getElementsByClassName('main-panel')[0]
    ...
    function transform (selector, classOne, classTwo, scrollableElement) {
      const st = document.getElementsByClassName(scrollableElement)[0].scrollTop
      const selectorEl = document.getElementsByClassName(selector)
...
er = {
  ...
  setChosenDateWithSelected (elem) {
    const positionId = elem.getAttribute('data-target')
    ...
    const monthsSelected = document.getElementsByClassName('monthPickerMonthSelected')
  ...
  tableSortMechanism () {
    const tableSortMech = window.sortingTransTable
...
function populateCurrentDate (date) {
  ...
  for (let i = date.length - 1; i >= 0; i--) {
    const presentDate = date[i].dateId
    const presentDateAsDate = new Date(presentDate.substring(5, presentDate.length))
...
function fetchJSONForCategories (data) {
  ...
  for (let count = 0, length = window.defaultCategories.length; count < length; count++) {
    const value = window.defaultCategories[count]
    ...
    const categoryData = dataNameMap[value.name]
  ...
  for (const [key, value] of Object.entries(dataNameMap)) {
...
function assignCategoryId (data) {
  ...
  if (isNotEmpty(window.categoryMap[categoryName])) {
    const category = {}
    const categoryTotal = window.categoryMap[categoryName].categoryTotal
  ...
  const children = iterateElement.children
