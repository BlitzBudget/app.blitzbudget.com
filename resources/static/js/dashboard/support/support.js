...
(function scopeWrapper ($) {
  ...
  function autocomplete (inp, arr, scrollWrapEl) {
    ...
    function inputTriggerAutoFill (e) {
      ...
      let len = arr.length
      let upperVal; let startsWithChar; let regVal
      ...
      for (let i = 0; i < len; i++) {
        ...
        if (startsWithChar) {
          ...
          const startPos = regVal.exec(arr[i].title.toUpperCase()).index
  ...
  function loadAutoCompleteModuleOnSwal () {
    ...
    const faq = []
    const categoryInformation = window.categoryInfo[getLanguage()]
    for (let i = 0, len = categoryInformation.length; i < len; i++) {
      const categoryInfoItem = categoryInformation[i]
      const subCategoryArr = categoryInfoItem.subCategory
      ...
      for (let j = 0, length = subCategoryArr.length; j < length; j++) {
        // FAQ
        const subCategoryItem = subCategoryArr[j]

        // Do not populate the item if it redirects to a static page
        const anchorHref = subCategoryItem.url
  ...
  function populateSubCategoryInfo (result) {
    ...
    const categoryInfo = window.categoryInfo[getLanguage()]

    // Category Information iteration
    for (let i = 0, len = categoryInfo.length; i < len; i++) {
      const category = categoryInfo[i]
  ...
  function populateSubCategory (category) {
    const subCategoryDiv = document.createDocumentFragment()
    const subCategoryNav = category.subCategory
    ...
    for (let i = 0, len = subCategoryNav.length; i < len; i++) {
      const subCategoryNavItem = subCategoryNav[i]

      // Do not populate the item if it redirects to a static page
      const anchorHref = subCategoryNavItem.url
      if (anchorHref.indexOf('http://app.blitzbudget.com') == 0 ||
                anchorHref.indexOf('https://app.blitzbudget.com') == 0 ||
                anchorHref.indexOf('http://www.blitzbudget.com') == 0 ||
                anchorHref.indexOf('https://www.blitzbudget.com') == 0 ||
      ...
      const anchorArticle = document.createElement('a')
  ...
  function populateArticle (content) {
    const articleDiv = document.createDocumentFragment()
    ...
    for (let i = 0, len = content.length; i < len; i++) {
      ...
      const tag = document.createElement(contentItem.tag)
  ...
  function populateBreadcrumb (result) {
    const breadcrumbDiv = document.createDocumentFragment()
    const breadcrumbSC = result.breadcrumb
    ...
    // Bread crumb 0
    const breadcrumbAnchor = breadcrumbSC[0]
    const anchorZero = document.createElement('a')
    ...
    for (let i = 1, len = breadcrumbSC.length; i < len; i++) {
      ...
      const breadcrumbAnchor = breadcrumbSC[i]
      const anchorOther = document.createElement('a')
  ...
  function buildMaterialSpinner () {
    ...
    const divMaterialSpinner = document.createElement('div')
  ...
  function needMoreInformation (url) {
    const articleFailure = document.getElementById('article-ratings-failure')
  ...
  function showSuccessMessage () {
    const articleSuccess = document.getElementById('article-ratings-success')
  ...
  function checkRatingInLS (url) {
    const ratingLS = JSON.parse(localStorage.getItem(SUPPORT_CONSTANTS.ratingLS))
    if (isEmpty(ratingLS)) {
      return false
    }
    ...
    for (let i = 0, len = ratingLS.length; i < len; i++) {
      const articleRating = ratingLS[i]
