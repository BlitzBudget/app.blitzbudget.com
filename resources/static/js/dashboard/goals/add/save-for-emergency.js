...
(function scopeWrapper ($) {
  ...
  $('body').on('click', '#data-picker-month-emergency .dropdown-item', function (event) {
    const currentMonth = (Number(this.dataset.month) + 1)
  ...
  $('body').on('change paste keyup', '#average-expense-emergency', function () {
    ...
    /*
         ...
         */
    const monthlyContribution = document.getElementById('your-monthly-contribution').value
  ...
  $('body').on('click', '#save-emergency-goals.emergency-add', function (event) {
    // Choose Planned Date
    const plannedDate = document.getElementById('planned-date-emergency')
    ...
    /*
         ...
         */
    const walletId = window.currentUser.walletId
    const goalType = window.goalType.emergency
    const targetAmount = averageExp * window.emergencyFundMonths.noUiSlider.get().substring(0, 2)
    const targetDate = newDate
    const targetType = 'Wallet'
    const targetId = window.currentUser.walletId
...
function calculateTotalPlannedDate () {
  ...
  const totalEmergencyFund = averageExpenseEmergency * Number(window.emergencyFundMonths.noUiSlider.get().substring(0, 2))
