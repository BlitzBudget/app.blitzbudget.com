...
(function scopeWrapper($) {
	...
	function fetchTransactions() {
		jQuery.ajax({
			url: CUSTOM_DASHBOARD_CONSTANTS.transactionAPIUrl + CUSTOM_DASHBOARD_CONSTANTS.transactionFetchCategoryTotal + currentUser.walletId + CUSTOM_DASHBOARD_CONSTANTS.dateMeantFor + chosenDate + CUSTOM_DASHBOARD_CONSTANTS.updateBudgetFalseParam,
			beforeSend: function(xhr){xhr.setRequestHeader("Authorization", authHeader);},
            type: 'GET',
	...
	function updateBudgetVisualization(createNewChart) {
		...
		if(isNotEmpty(userBudgetCacheKeys)) {
			animateValue(totalBudgetedCategoriesDiv, 0, userBudgetCacheKeys.length, '' ,2000);
			...
			if(isNotEmpty(categoryTotalKeys)) {
				...
				animateValue(toBeBudgetedDiv, 0, toBeBudgetedAvailable, '' ,2000);				
	...
	$('#budgetAmount').on('click', '.deleteBudget' , function(e) {
		...
		let ajaxData = {};
   		...
   		ajaxData.url = CUSTOM_DASHBOARD_CONSTANTS.budgetAPIUrl + currentUser.walletId + '/' + categoryId + CUSTOM_DASHBOARD_CONSTANTS.dateMeantFor + chosenDate + CUSTOM_DASHBOARD_CONSTANTS.deleteOnlyAutoGeneratedFalseParam;
	...
	$('#budgetAmount').on('click', '#copyPreviousMonthsBudget' , function(e) {
		...
		let ajaxData = {};
   		ajaxData.isAjaxReq = true;
   		ajaxData.type = "POST";
   		ajaxData.url = CUSTOM_DASHBOARD_CONSTANTS.budgetAPIUrl + CUSTOM_DASHBOARD_CONSTANTS.budgetCopyBudgetUrl + currentUser.walletId;
   		ajaxData.dataType = "json";
   		ajaxData.contentType = "application/x-www-form-urlencoded; charset=UTF-8";
   		ajaxData.values = values;
	...
	function fetchAllDatesWithUserBudgetData() {
		...
		let ajaxData = {};
   		ajaxData.isAjaxReq = true;
   		ajaxData.type = 'GET';
   		ajaxData.url = CUSTOM_DASHBOARD_CONSTANTS.budgetAPIUrl + CUSTOM_DASHBOARD_CONSTANTS.budgetFetchAllDates + currentUser.walletId;
   		ajaxData.onSuccess = function(dates) {
	...
	$( "#budgetAmount" ).on( "change", ".categoryOptions" ,function() {
		...
		let ajaxData = {};
   		ajaxData.isAjaxReq = true;
   		ajaxData.type = "POST";
   		ajaxData.url = CUSTOM_DASHBOARD_CONSTANTS.budgetAPIUrl + CUSTOM_DASHBOARD_CONSTANTS.changeBudgetUrl + currentUser.walletId;
   		ajaxData.dataType = "json";
   		ajaxData.contentType = "application/x-www-form-urlencoded; charset=UTF-8";
   		ajaxData.values = values;
   		ajaxData.onSuccess = function(userBudget){
