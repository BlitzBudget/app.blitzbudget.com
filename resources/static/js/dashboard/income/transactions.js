...
(function scopeWrapper($) {
	...
	function updateBudgetForIncome() {
		...
		ajaxData.onFailure = function(thrownError) {
		  manageErrors(thrownError, "Unable to fetch budget for the selected month. Please try again later!");
        }
	...
	function deleteAllAutoGeneratedUserBudget() {
		...
		ajaxData.dataType = "json";
		ajaxData.onFailure = function(thrownError) {
		  manageErrors(thrownError, "Unable to delete the budget associated with the transaction.");
        }
		jQuery.ajax({
			url: ajaxData.url,
			beforeSend: function(xhr){xhr.setRequestHeader("Authorization", authHeader);},
				 ...
				 Swal.fire({	
			                ...
			                buttonsStyling: false,
			            	 ...
			            	 if (result.value) {
								 ...
								 ajaxData.onFailure = function (thrownError) {
								 	manageErrors(thrownError, 'Unable to delete the transactions');
		                         }
	...
	$( "#transactionsTable" ).on( "change", ".tableRowForSelectCategory" ,function() {
		...
		if(isNotEmpty(classList)) {
			...
			ajaxData.onFailure = function (thrownError) {
				manageErrors(thrownError, 'Unable to change the category.');
            }
	...
	function postNewDescriptionToUserTransactions(element) {
		...
		ajaxData.onFailure = function (thrownError) {
			manageErrors(thrownError, 'Unable to change the description.');
        }
	...
	function postNewAmountToUserTransactions(element){
		...
		if(previousText != enteredText){
			...
			ajaxData.onFailure = function (thrownError) {
				manageErrors(thrownError, 'Unable to change the transacition amount.');
	        }
			$.ajax({
		          type: ajaxData.type,
		          url: ajaxData.url,
	...
	$( "#transactionsTable" ).on( "click", ".addTableRowListener" ,function(event) {
		 ...
		 ajaxData.onFailure = function (thrownError) {
		 	manageErrors(thrownError, 'Unable to add a new transaction.');
          }
	...
	function recalculateCategoryTotalAmount() {
		...
		ajaxData.onFailure = function(thrownError) {
			manageErrors(thrownError, 'Unable to fetch transaction total.');
	    }
		// Load all user transaction from API
		jQuery.ajax({
			url: ajaxData.url,
