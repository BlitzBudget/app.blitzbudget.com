...
(function scopeWrapper ($) {
  ...
  const errorAddingTransactionDiv = '<div class="row ml-auto mr-auto"><i class="material-icons red-icon">highlight_off</i><p class="margin-bottom-zero red-icon margin-left-five">'
  ...
  function initialLoadOfTransactions () {
    ...
    const tttitle = isNotEmpty(window.translationData) ? window.translationData.transactions.dynamic.add.tooltip : 'Add Transactions'
    ...
    $('#GSCCModal').on('show.bs.modal', function () {
      ...
      const expenseOptGroup = document.getElementById('expenseSelection')
      const incomeOptgroup = document.getElementById('incomeSelection')
  ...
  function registerTransaction (event, addTransactionsButton) {
    ...
    const recurrence = document.querySelector('.register-recurrence.active')
    const recurrenceValues = ['NEVER', 'WEEKLY', 'BI-MONTHLY', 'MONTHLY']

    // If the recurrence is not empty then assign the checked one
    const recurrenceValue = recurrence.getAttribute('data-target')
    ...
    const categoryOptions = document.getElementById('categoryOptions').getAttribute('data-chosen')
    const tagsChosen = document.querySelectorAll('#add-transaction-tags .badge')
    const tagsArray = []
    ...
    /*
         ...
         */
    const badgeText = document.getElementById('add-transaction-value').value
    ...
    if (notIncludesStr(categoryOptions, 'Category#')) {
      ...
      if (isEmpty(chosenCategory)) {
        ...
        for (const [key, value] of Object.entries(window.categoryMap)) {
          const categoryItem = window.categoryMap[key]
      ...
      const chosenCategory = window.categoryMap[categoryOptions]
    ...
    ajaxData.onSuccess = function (result) {
      ...
      const transaction = result['body-json']
      // Assign Category Id
      ...
      const successMessageDocument = document.getElementById('successMessage')
    ...
    ajaxData.onFailure = function (data) {
      ...
      const responseError = data.responseJSON
  ...
  $('body').on('click', '#categoryOptions .dropdown-item', function (event) {
    const dropdownValue = this.lastElementChild.value
    const categoryOption = document.getElementById('categoryOptions')
  ...
  function populateCategorySort (result) {
    if (isEmpty(result.Transaction) && isEmpty(result.Category)) {
      const transactionsTable = document.getElementById(replaceTransactionsId)
      ...
      const categoryInfoTable = document.getElementsByClassName('categoryInfoTable')
      // Replace HTML with Empty
  ...
  function replacePieChartWithMSpinner () {
    ...
    const chartFinPosition = document.getElementById('chartFinancialPosition')
    // Replace HTML with Empty
  ...
  function populateAccountTableInformation (userTransactionsList) {
    if (isEmpty(userTransactionsList)) {
      ...
      const accountInfoTable = document.getElementsByClassName('accountInfoTable')
      // Replace HTML with Empty
  ...
  function populateRecentTransInfo (userTransactionsList) {
    ...
    const recentTransactionsDiv = document.getElementById(recentTransactionsId)
    const recentTransactionsFragment = document.createDocumentFragment()
    ...
    if (isEmpty(userTransactionsList)) {
      ...
      const resultKeySet = Object.keys(userTransactionsList)
      for (let countGrouped = 0; countGrouped < resultKeySet.length; countGrouped++) {
        const key = resultKeySet[countGrouped]
        const userTransaction = userTransactionsList[key]
        const creationDate = new Date(userTransaction.creation_date)
  ...
  function buildTransactionRow (userTransaction, idName) {
    ...
    // Category Map
    const categoryMapForUT = categoryMap[userTransaction.category]
  ...
  $('body').on('click', '.categorySortGrp', function (e) {
    ...
    const materialArrow = this.firstElementChild.firstElementChild
    materialArrow.classList.toggle('rotateZero')
    materialArrow.classList.toggle('rotateNinty')
    const childElementWrappers = parentWrapper.childNodes
  ...
  function buildCategoryHeader (category) {
    const categoryData = window.categoryMap[category]
    ...
    const categoryTit = document.createElement('div')
    ...
    // Account Balance
    const categoryBalance = document.createElement('div')
  ...
  function populateTransactionsByCategory (userTransactionsList) {
    ...
    for (let i = 0, len = userTransactionsList.length; i < len; i++) {
      const userTransaction = userTransactionsList[i]
      window.transactionsCache[userTransaction.transactionId] = userTransaction
      const category = userTransaction.category
  ...
  function popTransByAccWOAJAX () {
    ...
    const accountAggreDiv = document.getElementById('accSortedTable')
    const recentTransactionsFragment = document.createDocumentFragment()
    const createdAccIds = []
    ...
    // Transactions Cache
    for (const key of Object.keys(window.transactionsCache)) {
      const userTransaction = window.transactionsCache[key]
      const accountId = userTransaction.account
    ...
    const accHeadFrag = document.createDocumentFragment()
    // Iterate all bank accounts
    for (let i = 0, length = window.allBankAccountInfoCache.length; i < length; i++) {
      const bankAcc = window.allBankAccountInfoCache[i]
      ...
      if (notIncludesStr(createdAccIds, bankAcc.accountId)) {
        ...
        const accountHeaderNew = er_a.buildAccountHeader(bankAcc)
        const accBal = accountHeaderNew.getElementById('accountBalance-' + bankAcc.accountId)
...
tr = {
  ...
  updatePieChartTransactions (totalIncomeTransactions, totalExpensesTransactions, totalAvailableTransactions) {
    ...
    if (totalIncomeTransactions === 0 && totalExpensesTransactions === 0) {
      const empty = isNotEmpty(window.translationData) ? window.translationData.transactions.dynamic.chart.empty : 'Please fill in adequare data to build a chart'
      replaceHTML('legendPieChart', empty)
    } else if (totalIncomeTransactions < Math.abs(totalExpensesTransactions)) {
      const exp = isNotEmpty(window.translationData) ? window.translationData.transactions.dynamic.chart.expense : 'Total Income & Total Overspent as a percentage of Total Expense'
      const overspent = isNotEmpty(window.translationData) ? window.translationData.transactions.dynamic.chart.overspent : 'Total Overspent'
      replaceHTML('legendPieChart', exp)
      replaceHTML('totalAvailableLabel', overspent)
      const totalDeficitAsPercentageOfExpense = round(((totalAvailableTransactions / totalExpensesTransactions) * 100), 1)
      const totalIncomeAsPercentageOfExpense = round(((totalIncomeTransactions / totalExpensesTransactions) * 100), 1)
      ...
      const income = isNotEmpty(window.translationData) ? window.translationData.transactions.dynamic.chart.income : 'Total Spent & Total Available as a percentage of Total Income'
      const available = isNotEmpty(window.translationData) ? window.translationData.transactions.dynamic.chart.available : 'Total Available'
      replaceHTML('legendPieChart', income)
      replaceHTML('totalAvailableLabel', available)
      const totalAvailableAsPercentageOfIncome = round(((totalAvailableTransactions / totalIncomeTransactions) * 100), 1)
      const totalExpenseAsPercentageOfIncome = round(((totalExpensesTransactions / totalIncomeTransactions) * 100), 1)
      // labels: [INCOME,EXPENSE,AVAILABLE]
      dataPreferences = {
  ...
  buildPieChart (dataPreferences, id) {
    ...
    const inc = window.translationData.transactions.dynamic.chart.labels.income
    const spent = window.translationData.transactions.dynamic.chart.labels.spent
    const avai = window.translationData.transactions.dynamic.chart.labels.available
    const oversp = window.translationData.transactions.dynamic.chart.labels.overspent
    const labels = [inc, spent, avai, oversp]
    ...
    if (isNotEmpty(dataPreferences)) {
      window.transactionsChart = new Chartist.Pie('#' + id, dataPreferences, optionsPreferences).on('draw', function (data) {
        ...
        const incomeAmount = document.getElementById('totalIncomeTransactions')
        const expenseAmount = document.getElementById('totalExpensesTransactions')
        const totalAvailable = document.getElementById('totalAvailableTransactions')
  ...
  loadCategoriesForTransaction (dropdownId, expenseId, incomeId) {
    ...
    const defaultCategory = window.defaultCategories[1]
    ...
    // Load Expense category and income category
    const expenseSelectionDiv = document.getElementById(expenseId)
    while (expenseSelectionDiv.firstChild) {
      expenseSelectionDiv.removeChild(expenseSelectionDiv.lastChild)
    }
    const incomeSelectionDiv = document.getElementById(incomeId)
  ...
  sortTransactionsByAccount () {
    ...
    const popAccInfoTab = document.getElementsByClassName('accountInfoTable')
    if (isEmpty(popAccInfoTab)) {
      ...
      for (let i = 0, len = popAccInfoTab.length; i < len; i++) {
        const elementInArray = popAccInfoTab[i]
