...
(function scopeWrapper ($) {
  ...
  $('body').on('dragenter', '#futureTransactionsTable .recurTransInfoTable', function (e) {
    ...
    const closestParentWrapper = e.target.closest('.recurTransInfoTable')
  ...
  function handleDragStart (e) {
    ...
    const recurTransInfoTables = document.getElementsByClassName('recurTransInfoTable')
    for (let i = 0, l = recurTransInfoTables.length; i < l; i++) {
      ...
      /*
             ...
             */
      const recurInfoTable = recurTransInfoTables[i]
      const childElementWrappers = recurInfoTable.childNodes

      // Recur Transactions contains d-none
      if (childElementWrappers[1].classList.contains('d-none')) {
        // Rotate the arrow
        const arrowIndicator = recurInfoTable.firstElementChild.firstElementChild.firstElementChild
  ...
  function handleDrop (e) {
    ...
    if (dragSrcEl != closestParentWrapper) {
      ...
      const recurTransId = e.originalEvent.dataTransfer.getData('text/plain')
      const a = document.getElementById(recurTransId)
  ...
  function updateTransactionWithRecurrence (recurringTransactionId, recurrence, oldRecurrence) {
    ...
    ajaxData.onSuccess = function (result) {
      const recurrenceData = result['body-json']
      // Remove empty entries for the recurrence
      const emptyRecurrenceItem = document.getElementById('emptyRecurrenceItem-' + recurrence)
      if (isNotEmpty(emptyRecurrenceItem)) {
        emptyRecurrenceItem.remove()
      }
      // Replace with Empty Recurrence
      const oldRecurrenceEl = document.getElementById('recurTransSB-' + oldRecurrence)
      const oldRecentTransactionEntry = oldRecurrenceEl.getElementsByClassName('recentTransactionEntry')
