...
(function scopeWrapper ($) {
  ...
  $('body').on('dragenter', '#transactionsTable .categoryInfoTable', function (e) {
    ...
    const closestParentWrapper = e.target.closest('.categoryInfoTable')
  ...
  function handleDragStart (e) {
    ...
    const categoryInfoTables = document.getElementsByClassName('categoryInfoTable')
    for (let i = 0, l = categoryInfoTables.length; i < l; i++) {
      ...
      /*
             ...
             */
      const categoryInfoTable = categoryInfoTables[i]
      const childElementWrappers = categoryInfoTable.childNodes

      // Recur Transactions contains d-none
      if (childElementWrappers[1].classList.contains('d-none')) {
        // Rotate the arrow
        const arrowIndicator = categoryInfoTable.firstElementChild.firstElementChild.firstElementChild
  ...
  function updateTransactionWithCatId (transactionId, categoryId, oldCategoryId) {
    ...
    ajaxData.onSuccess = function (result) {
      ...
      const oldCatDiv = document.getElementById('categoryBalance-' + oldCategoryId)
      const oldCatBal = window.categoryMap[oldCategoryId].categoryTotal
      ...
      const emptyEntriesNewCat = document.getElementById('emptyCategoryItem-' + categoryId)
      if (isNotEmpty(emptyEntriesNewCat)) {
        emptyEntriesNewCat.remove()
      }
      // Replace with Empty Cat Trans
      const oldCatTable = document.getElementById('categorySB-' + oldCategoryId)
      const oldRecentTransactionEntry = oldCatTable.getElementsByClassName('recentTransactionEntry')
