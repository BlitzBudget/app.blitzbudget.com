...
(function scopeWrapper ($) {
  ...
  $('body').on('dragenter', '#accSortedTable .accountInfoTable', function (e) {
    ...
    const closestParentWrapper = e.target.closest('.accountInfoTable')
    ...
    const recentTransactionEntry = closestParentWrapper.getElementsByClassName('recentTransactionEntry')
    if (recentTransactionEntry.length > 0) {
      for (let i = 0, l = recentTransactionEntry.length; i < l; i++) {
        const recTransEntry = recentTransactionEntry[i]
  ...
  function defineHrElement (target) {
    let dropped = null
    const positionOfDrag = document.getElementsByClassName('positionOfDrag')
    ...
    const nearestTrans = target.closest('.recentTransactionEntry')
  ...
  function handleDragStart (e) {
    ...
    const accountInfoTables = document.getElementsByClassName('accountInfoTable')
    for (let i = 0, l = accountInfoTables.length; i < l; i++) {
      dragsterList.push(new Dragster(accountInfoTables[i]))
      /*
             * Unhide the table elements
             */
      const accountInfoTable = accountInfoTables[i]
      const childElementWrappers = accountInfoTable.childNodes

      // Account Info Table if it contains none
      if (childElementWrappers[1].classList.contains('d-none')) {
        // Rotate the arrow
        const arrowIndicator = accountInfoTable.firstElementChild.firstElementChild.firstElementChild
        arrowIndicator.classList.toggle('rotateZero')
        arrowIndicator.classList.toggle('rotateNinty')
      }

      // Unhide all the child nodes
      for (let i = 1, len = childElementWrappers.length; i < len; i++) {
        const childElementWrapper = childElementWrappers[i]
        const childClassList = childElementWrapper.classList
  ...
  function handleDrop (e) {
    ...
    if (dragSrcEl != closestParentWrapper) {
      ...
      const transId = e.originalEvent.dataTransfer.getData('text/plain')
      const a = document.getElementById(transId)
  ...
  function updateTransactionWithAccId (transactionId, accountId, oldAccountId) {
    ...
    ajaxData.onSuccess = function (result) {
      const userTransaction = result['body-json']
      // Fetch the current account balance
      const oldAccDiv = document.getElementById('accountBalance-' + oldAccountId)
      const accDiv = document.getElementById('accountBalance-' + accountId)
      let oldAccBal = 0
      let accBal = 0
      ...
      const emptyEntriesNewAcc = document.getElementById('emptyAccountEntry-' + accountId)
      if (isNotEmpty(emptyEntriesNewAcc)) {
        emptyEntriesNewAcc.remove()
      }
      // Replace with Empty Acc Trans
      const oldAccTable = document.getElementById('accountSB-' + oldAccountId)
      const oldRecentTransactionEntry = oldAccTable.getElementsByClassName('recentTransactionEntry')
