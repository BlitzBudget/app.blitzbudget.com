...
(function scopeWrapper ($) {
  ...
  $('body').on('click', '.accountInfoTable .recentTransactionDateGrp', function (e) {
    // Account modal id
    const accInfoTable = this.closest('.accountInfoTable')
    const accountId = accInfoTable.getAttribute('data-target')
    // Set the current account
    currentAccountId = accountId
    const accountModal = document.getElementById('accountInformationMdl').classList
    // Fetch the total number of transactions for the account
    const recentTransactionEntry = accInfoTable.getElementsByClassName('recentTransactionEntry')
    // Set the number of transactions if present
    const numberOfTransactionsDiv = document.getElementById('numberOfTransInAcc')
    ...
    const accTransEntry = this.parentNode.getElementsByClassName('accTransEntry')
    ...
    const emptyTransInAcc = document.getElementById('emptyAccountEntry-' + accountId)
  ...
  function submitAmountChange (element) {
    ...
    if (previousText != enteredText) {
      ...
      ajaxData.onSuccess = function (bankAccount) {
        ...
        const formattedBudgetAmount = formatToCurrency(bankAccount.accountBalance)
      ...
      ajaxData.onFailure = function (thrownError) {
        ...
        const formattedAccountAmount = formatToCurrency(previousText)
  ...
  function deleteBBAccount () {
    const accountLabelInModal = document.getElementById('accountLabelInModal')
  ...
  function fetchEmptyTableMessage () {
    ...
    // Row 4
    const descriptionTableCell = document.createElement('div')
    ...
    // Row 5
    const amountTableCell = document.createElement('div')
    amountTableCell.className = 'd-table-cell'
    emptyTableRow.appendChild(amountTableCell)

    // Row 6
    const budgetTableCell = document.createElement('div')
