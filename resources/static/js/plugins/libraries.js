...
function calcPage () {
  ...
  const sideBarId = currentActiveSideBar.id
...
function replaceWithCurrency (wallet) {
  const currencySymbolDivs = document.getElementsByClassName('currentCurrencySymbol')
...
function manageErrors (thrownError, message, ajaxData) {
  const responseError = thrownError.responseJSON
