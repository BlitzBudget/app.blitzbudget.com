<template>
    <form>
        <card footer-classes="text-left">
            <div slot="header">
                <h4 class="card-title mb-0">{{ $t('user.reset-account.title') }}</h4>
                <small class="description">{{ $t('user.reset-account.description') }}</small>
            </div>
            <div>
                <base-input v-validate="'required|min:8'" name="password" :error="getError('password')"
                    ...
                    :label="$t('user.reset-account.password')">
                </base-input>

                <div class="small form-category">{{ $t('user.reset-account.required-fields') }}</div>
            </div>

            <template slot="footer">
                <base-button native-type="submit" @click.native.prevent="validate" type="primary">{{
                        $t('user.reset-account.button')
                }}</base-button>
            </template>
        </card>
    </form>
