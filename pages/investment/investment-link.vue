...
export default {
    ...
    methods: {
        ...
        async getInvestments(investmentRuleResponse) {
            ...
            let investmentIds = this.getInvestmentIds(investmentRuleResponse);
            // Fetch the current user ID
            let userId = this.$authentication.fetchCurrentUser(this).financialPortfolioId;
            await this.$axios.$post(process.env.api.investment.batch, {
                user_id: userId,
        ...
        getInvestmentIds(investmentRuleResponse) {
            let investmentIds = [];

            for (let i = 0, length = investmentRuleResponse.length; i < length; i++) {
                let investmentRule = investmentRuleResponse[i];
                investmentIds.push(investmentRule.investment_id);
            }
